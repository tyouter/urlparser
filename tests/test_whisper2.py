...
print('=' * 60)
print('Test 2: Whisper Audio Transcription (with HF mirror)')
print('=' * 60)

from urlparser.transcriber import WhisperTranscriber
...
transcriber = WhisperTranscriber(model_size='large', device='cpu')
...
output_file = os.path.join(OUTPUT_DIR, '02_whisper_transcription.md')
lines = ['# Whisper 音频转录测试', '']
now_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
lines.append(f'> 测试时间: {now_str}')
lines.append(f'> 测试URL: {TEST_URL}')
lines.append(f'> 引擎: Whisper (faster-whisper large, int8)')
